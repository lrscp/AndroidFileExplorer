...
public final class FileListingService {
    ...
    private final static Pattern sApkPattern = Pattern
            .compile(".*\\.apk", Pattern.CASE_INSENSITIVE); //$NON-NLS-1$
    ...
    private String mLogLevel = "warn";// "verbose";
    ...
    public final static class FileEntry {
        ...
        private final static Pattern sEscapePattern = Pattern.compile("([\\\\()*+?\"'#/\\s])"); //$NON-NLS-1$
        ...
        public FileEntry getParent() {
            return new FileEntry(getParent(path));
        }
         ...
         *            shell command line.
         */
        // protected void fillPathBuilder(StringBuilder pathBuilder, boolean
        // escapePath) {
        // if (isRoot) {
        // return;
        // }
        ...
        protected void fillPathSegments(ArrayList<String> list) {
            Log.e("fillPathSegments not implemented!!");
        }

        /**
         * Sets the internal app package status flag. This checks whether the
         * entry is in an app directory like /data/app or /system/app
         */
        private void checkAppPackageStatus() {
            ...
            if (type == TYPE_FILE && segments.length == 3 && isAppFileName()) {
                isAppPackage = DIRECTORY_APP.equals(segments[1])
                        && (DIRECTORY_SYSTEM.equals(segments[0]) || DIRECTORY_DATA
                                .equals(segments[0]));
            }
        }
        ...
        public static String getParentDirectory(String path) {
            ...
            if (index < 0 || path.equals(FILE_ROOT)) {
                return null;
         ...
         * @return
         */
        public static String pathJoin(String path, String children){
            String[] segs = children.split(FILE_SEPARATOR);
            for(String seg : segs){
                if(seg == null || seg.length() == 0 || seg.equals(".")) continue;
                if(seg.equals("..")) {
    ...
    private class LsReceiver extends MultiLineReceiver {
        ...
        public void processNewLines(String[] lines) {
            for (String line : lines) {
                ...
                if (entry == null) {
                    String parentPath = mParentEntry.path;
                    if (!mParentEntry.path.endsWith("/")) {
                        parentPath = mParentEntry.path + "/";
                    }
                    entry = new FileEntry(parentPath + name);
                }
                ...
                entry.group = group;
                if (objectType == TYPE_LINK) {
                    if(!info.startsWith(FILE_SEPARATOR)){
                        info = FileEntry.pathJoin(entry.getParentDirectory(), info);
                    }
                    entry.info = info;
                }
     ...
     * 
     * @see FileListingService#getChildren(com.android.ddmlib.FileListingService.FileEntry,
     *      boolean, com.android.ddmlib.FileListingService.IListingReceiver)
     */
     ...
     * {@link #getChildren(FileEntry, boolean, IListingReceiver)}. It is roughly
     * equivalent to calling getChildren(FileEntry, false, null)
     * 
     * @param entry
     *            The parent entry.
    ...
    public int delete(FileEntry entry, StringBuilder msg) {
        try {
            return mDevice.execShellCmdWithExitCode("rm -r " + entry.getFullEscapedPath(), msg);
    ...
    public int copyTo(FileEntry src, FileEntry dest, StringBuilder msg) {
        try {
            return mDevice.execShellCmdWithExitCode("busybox cp -R " + src.getFullEscapedPath() + " " + dest.getFullEscapedPath(), msg);
        } catch (Exception e) {
            e.printStackTrace();
        } 
    ...
    public int moveTo(FileEntry src, FileEntry dest, StringBuilder msg) {
        try {
            return mDevice.execShellCmdWithExitCode("busybox mv " + src.getFullEscapedPath() + " " + dest.getFullEscapedPath(), msg);
        } catch (Exception e) {
            e.printStackTrace();
        } 
